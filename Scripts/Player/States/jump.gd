...
func new_state(delta : float, change_state):
	...
	initial_y_velocity = GROUND_INITIAL_VELOCITY if last_state == P.State.GROUNDED else DOUBLE_INITIAL_VELOCITY
	velocity.y = initial_y_velocity
	y_decceleration = -initial_y_velocity / (GROUND_DECELERATION_TIME if last_state == P.State.GROUNDED else DOUBLE_DECELERATION_TIME)
	update(delta)
	...
	if(state_change_to != Player.State.JUMP):
		...
		match state_change_to:
			...
			Player.State.GROUNDED:
				...
				P.Grounded.new_state(delta, Player.State.JUMP)
			Player.State.KICK:
				pass
			Player.State.SLIDE:
