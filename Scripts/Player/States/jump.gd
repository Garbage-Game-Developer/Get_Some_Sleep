...
@export var GROUND_INITIAL_VELOCITY : float = 500.0		##	Initial speed (px * s) the player gets if jumping from the ground
...
@export var DOUBLE_INITIAL_VELOCITY : float = 400.0		##	Initial speed (px * s) the player gets if jumping from the air
...
func update(delta : float):
	...
	if(state_change_to != Player.State.GROUNDED):
		print("DEBUG - State changing to : ", state_change_to)
		#ACTIVE_STATE = false
		
		"""	Rework this to include all states for now, maybe move it into player parent, and don't forget to scroll down and see the other note """
